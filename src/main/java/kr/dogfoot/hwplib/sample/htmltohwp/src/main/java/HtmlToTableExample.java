...
public class HtmlToTableExample {
    ...
    private Map<String, List<List<TableCellDTO>>> parseTable(Document doc){
        int idtable = 1;
        Map<String, List<List<TableCellDTO>>> tableMap = new HashMap<>();
    ...
    private List<List<TableCellDTO>> parseList(Element table) {
        List<List<TableCellDTO>> returnRowDataList = new ArrayList<>();
        List<Map<Integer, TableCellDTO>> rowDataList = new ArrayList<>();
        ...
        for (Map<Integer, TableCellDTO> m : rowDataList) {
            TreeMap<Integer, TableCellDTO> tm = new TreeMap<>(m);
            Iterator<Integer> iteratorKey = tm.keySet().iterator();
            List<TableCellDTO> subList = new ArrayList<>();
